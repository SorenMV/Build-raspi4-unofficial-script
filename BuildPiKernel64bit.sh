...
cp -rf "$BUILD_ROOT"/firmware-raspbian/* "$BUILD_ROOT"/firmware-nonfree
...
cd "$BUILD_ROOT"
if [ ! -d "rpi-linux" ]; then
  git clone https://github.com/SorenMV/linux.git rpi-linux --single-branch --branch rpi-4.19.y --depth 1
  cd rpi-linux
  ...
  PATH=$PATH:$TOOLCHAIN/bin make -j8 O=./kernel-build/ ARCH=arm64 DTC_FLAGS="-@ -H epapr" CROSS_COMPILE=aarch64-linux-gnu-
  ...
  sudo make -j8 O=./kernel-build/ DEPMOD=echo MODLIB=./kernel-install/lib/modules/${KERNEL_VERSION} INSTALL_FW_PATH=./kernel-install/lib/firmware modules_install
  # % Create kernel headers
  sudo make -j8 O=./kernel-build/ INSTALL_HDR_PATH=./kernel-install/usr/src/linux-headers-${KERNEL_VERSION} headers_install
...
# MOUNT IMAGE
xzcat ubuntu-18.04.3-preinstalled-server-arm64+raspi3.img.xz > ubuntu-18.04.3-custom-arm64+raspi4.img
MountXZ=$(sudo kpartx -av ubuntu-18.04.3-custom-arm64+raspi4.img)
...
sudo umount /mnt
sudo kpartx -dv ubuntu-18.04.3-custom-arm64+raspi4.img
